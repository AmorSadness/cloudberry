...
struct XpuNumericShort
{
	uint16_t		n_header;				/* Sign + display scale + weight */
	NumericDigit	n_data[PG_MAX_DATA];	/* Digits */
};
typedef struct XpuNumericShort	XpuNumericShort;

struct XpuNumericLong
{
	uint16_t		n_sign_dscale;			/* Sign + display scale */
	int16_t			n_weight;				/* Weight of 1st digit	*/
	NumericDigit	n_data[PG_MAX_DATA];	/* Digits */
};
typedef struct XpuNumericLong	XpuNumericLong;

typedef union
{
	uint16_t		n_header;			/* Header word */
	XpuNumericLong		n_long;				/* Long form (4-byte header) */
	XpuNumericShort	n_short;			/* Short form (2-byte header) */
} XpuNumericChoice;

struct XpuNumericData
{
	uint32_t		vl_len_;		/* varlena header */
	XpuNumericChoice	choice;			/* payload */
};
typedef struct XpuNumericData	XpuNumericData;

#define XPU_NUMERIC_SIGN_MASK	0xC000
#define XPU_NUMERIC_POS			0x0000
#define XPU_NUMERIC_NEG			0x4000
#define XPU_NUMERIC_SHORT		0x8000
#define XPU_NUMERIC_SPECIAL		0xC000

#define XPU_NUMERIC_FLAGBITS(n_head)	((n_head) & XPU_NUMERIC_SIGN_MASK)
#define XPU_NUMERIC_IS_SHORT(n_head)	(XPU_NUMERIC_FLAGBITS(n_head) == XPU_NUMERIC_SHORT)
#define XPU_NUMERIC_IS_SPECIAL(n_head)	(XPU_NUMERIC_FLAGBITS(n_head) == XPU_NUMERIC_SPECIAL)

#define XPU_NUMERIC_EXT_SIGN_MASK	0xF000	/* high bits plus NaN/Inf flag bits */
#define XPU_NUMERIC_NAN				0xC000
#define XPU_NUMERIC_PINF			0xD000
#define XPU_NUMERIC_NINF			0xF000
#define XPU_NUMERIC_INF_SIGN_MASK	0x2000

#define XPU_NUMERIC_EXT_FLAGBITS(n_head) (n_head & XPU_NUMERIC_EXT_SIGN_MASK)
#define XPU_NUMERIC_IS_NAN(n_head)		(n_head == XPU_NUMERIC_NAN)
#define XPU_NUMERIC_IS_PINF(n_head)		(n_head == XPU_NUMERIC_PINF)
#define XPU_NUMERIC_IS_NINF(n_head)		(n_head == XPU_NUMERIC_NINF)
#define XPU_NUMERIC_IS_INF(n_head)		((n_head & ~XPU_NUMERIC_INF_SIGN_MASK) == XPU_NUMERIC_PINF)

#define XPU_NUMERIC_SHORT_SIGN_MASK		0x2000
#define XPU_NUMERIC_SHORT_DSCALE_MASK	0x1F80
#define XPU_NUMERIC_SHORT_DSCALE_SHIFT	7
#define XPU_NUMERIC_SHORT_DSCALE_MAX	(XPU_NUMERIC_SHORT_DSCALE_MASK >> \
									 XPU_NUMERIC_SHORT_DSCALE_SHIFT)
#define XPU_NUMERIC_SHORT_WEIGHT_SIGN_MASK 0x0040
#define XPU_NUMERIC_SHORT_WEIGHT_MASK	0x003F
#define XPU_NUMERIC_SHORT_WEIGHT_MAX	XPU_NUMERIC_SHORT_WEIGHT_MASK
#define XPU_NUMERIC_SHORT_WEIGHT_MIN	(-(XPU_NUMERIC_SHORT_WEIGHT_MASK+1))

#define XPU_NUMERIC_DSCALE_MASK         0x3FFF

INLINE_FUNCTION(uint32_t)
XPU_NUMERIC_NDIGITS(uint16_t n_head, uint32_t nc_len)
{
	return (XPU_NUMERIC_IS_SHORT(n_head)
			? (nc_len - offsetof(XpuNumericChoice, n_short.n_data))
			: (nc_len - offsetof(XpuNumericChoice, n_long.n_data)))
		/ sizeof(NumericDigit);
}

INLINE_FUNCTION(NumericDigit *)
XPU_NUMERIC_DIGITS(XpuNumericChoice *nc, uint16_t n_head)
{
	return XPU_NUMERIC_IS_SHORT(n_head) ? nc->n_short.n_data : nc->n_long.n_data;
}

INLINE_FUNCTION(int)
XPU_NUMERIC_SIGN(uint16_t n_head)
{
	if (XPU_NUMERIC_IS_SHORT(n_head))
		return ((n_head & XPU_NUMERIC_SHORT_SIGN_MASK) ? XPU_NUMERIC_NEG : XPU_NUMERIC_POS);
	if (XPU_NUMERIC_IS_SPECIAL(n_head))
		return XPU_NUMERIC_EXT_FLAGBITS(n_head);
	return XPU_NUMERIC_FLAGBITS(n_head);
}

INLINE_FUNCTION(uint32_t)
XPU_NUMERIC_DSCALE(XpuNumericChoice *nc, uint16_t n_head)
{
	if (XPU_NUMERIC_IS_SHORT(n_head))
		return ((n_head & XPU_NUMERIC_SHORT_DSCALE_MASK) >> XPU_NUMERIC_SHORT_DSCALE_SHIFT);
	return (__Fetch(&nc->n_long.n_sign_dscale) & XPU_NUMERIC_DSCALE_MASK);
}

INLINE_FUNCTION(int)
XPU_NUMERIC_WEIGHT(XpuNumericChoice *nc, uint16_t n_head)
{
	int			weight;

	if (XPU_NUMERIC_IS_SHORT(n_head))
	{
		weight = (n_head) & XPU_NUMERIC_SHORT_WEIGHT_MASK;
		if (n_head & XPU_NUMERIC_SHORT_WEIGHT_SIGN_MASK)
			weight |= ~XPU_NUMERIC_SHORT_WEIGHT_MASK;
	}
	else
	{
...
XPU_NUMERIC_NORMALIZE(int16_t *p_weight, int128_t *p_value)
{
					   ...
					   const varlena *addr)
{
	...
	{
		XpuNumericChoice *nc = (XpuNumericChoice *)VARDATA_ANY(addr);
		uint16_t	n_head = __Fetch(&nc->n_header);

		/* special case if NaN, +/-Inf */
		if (XPU_NUMERIC_IS_SPECIAL(n_head))
		{
			if (XPU_NUMERIC_IS_NAN(n_head))
				*p_kind = XPU_NUMERIC_KIND__NAN;
			else if (XPU_NUMERIC_IS_PINF(n_head))
				*p_kind = XPU_NUMERIC_KIND__POS_INF;
			else if (XPU_NUMERIC_IS_NINF(n_head))
		...
		{
			NumericDigit *digits = XPU_NUMERIC_DIGITS(nc, n_head);
			int16_t		weight  = XPU_NUMERIC_WEIGHT(nc, n_head) + 1;
			int			ndigits = XPU_NUMERIC_NDIGITS(n_head, len);
			int128_t	value = 0;
			...
			if (XPU_NUMERIC_SIGN(n_head) == XPU_NUMERIC_NEG)
				value = -value;
...
{
	XpuNumericData	   *numData = (XpuNumericData *)buffer;
	XpuNumericLong	   *numBody = &numData->choice.n_long;
	...
	{
		len = offsetof(XpuNumericData, choice.n_header) + sizeof(uint16_t);
		if (buffer)
		{
			XpuNumericChoice *nc = (XpuNumericChoice *)buffer;

			if (kind == XPU_NUMERIC_KIND__POS_INF)
				nc->n_header = XPU_NUMERIC_PINF;
			else if (kind == XPU_NUMERIC_KIND__POS_INF)
				nc->n_header = XPU_NUMERIC_NINF;
			else
				nc->n_header = XPU_NUMERIC_NAN;
			SET_VARSIZE(nc, len);
		}
		return len;
	}
	...
	len = (offsetof(XpuNumericData, choice.n_long.n_data)
			  ...
			  uint8_t b_kind,  int16_t b_weight,  int128_t b_value)
{
		...
		b_kind == XPU_NUMERIC_KIND__VALID)
	{
		...
		a_value += b_value;
		XPU_NUMERIC_NORMALIZE(&a_weight, &a_value);
			  ...
			  uint8_t b_kind,  int16_t b_weight,  int128_t b_value)
{
		...
		b_kind == XPU_NUMERIC_KIND__VALID)
	{
		...
		*r_value  = a_value * b_value;
		XPU_NUMERIC_NORMALIZE(r_weight, r_value);
	}
	else
	{
		assert(a_kind != XPU_NUMERIC_KIND__VARLENA &&
			  ...
			  uint8_t b_kind,  int16_t b_weight,  int128_t b_value)
{
		...
		b_kind == XPU_NUMERIC_KIND__VALID)
	{
		...
		if (negative)
			ival = -ival;
		XPU_NUMERIC_NORMALIZE(&weight, &ival);
		*r_kind   = XPU_NUMERIC_KIND__VALID;
		*r_weight = weight;
		*r_value  = ival;
	}
