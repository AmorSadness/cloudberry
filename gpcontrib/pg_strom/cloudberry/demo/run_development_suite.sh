...
export PGOPTIONS="${PGOPTIONS:-} -c pg_strom.enable_gpujoin=off -c pg_strom.enable_gpusort=off -c pg_strom.cloudberry_enable_extended_agg=off -c pg_strom.cloudberry_enable_unfiltered_agg=off -c pg_strom.cloudberry_enable_redistribute_final=off -c pg_strom.cloudberry_enable_count_types=off -c pg_strom.cloudberry_enable_host_input=off -c pg_strom.cloudberry_enable_cpu_filter=off -c pg_strom.cloudberry_enable_heap_partition=off"
...
run gpusort env PGSTROM_RESULTS="$results/gpusort-artifacts" \
    python3 "$demo_dir/run_gpusort_regression.py"
